...
def file_content(server_id):
    ...
    subpath = request.args.get('path', '').replace('..', '')
...
def create_folder(server_id):
    ...
    subpath = data.get('path', '').replace('..', '')
    target = os.path.join(SERVERS[server_id]['path'], subpath, name)
...
def rename_file(server_id):
    if server_id not in SERVERS:
        return jsonify({'error': 'Server not found'}), 404
    data = request.get_json() or request.form
    old_name = data.get('old_name', '').replace('..', '')
    new_name = data.get('new_name', '').replace('..', '')
    subpath = data.get('path', '').replace('..', '')
    base_path = SERVERS[server_id]['path']
...
def upload_file(server_id):
    if server_id not in SERVERS:
        return jsonify({'error': 'Server not found'}), 404
    subpath = request.form.get('path', '').replace('..', '')
    file = request.files.get('file')
